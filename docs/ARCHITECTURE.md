...
measured robot state. Calibrated RGB-D adds synchronized depth and camera geometry;
the actor may query up to eight displayed pixels for visible world-frame surface
coordinates. This reduces monocular scale and frame ambiguity without exposing object
identities, object centers, grasp targets or simulator object state. A bounded feedback
adapter executes physical pose requests in LIBERO. The separate improvement agent reads
development evidence and proposes a reusable operating guide, with an evidence-backed
hypothesis and behavior prediction. The coordinator evaluates fresh candidate episodes
and selects only comparable, strict success-count improvements. Only LIBERO's predicate
establishes success.
...
1. Closed-loop translation and usable orientation control through the existing
   ...
   and achieved pose, residual error, step interval and stop status. A public Inspect
   controller preserves normally executing chunks, but discards their queued tail after
   measured `stalled` or `motion_limit` feedback and replans from a fresh observation.
...
The acting-policy adapter retains the stock action implementation and validation. Its
schema wording states the rot6d axis convention, bounded waypoint behavior and measured
outcomes explicitly, and adds `locate_pixels` as a non-moving sensor query. Each query
uses the depth image, intrinsics and camera-to-world transform from the same observation;
moving wrist-camera calibration is therefore observation-local. Invalid, background or
out-of-frame pixels return no world target. These measurements provide better geometric
evidence, but do not establish a grasp, task completion or learned improvement.
...
does not imply successful learning or cross-task generalization. The current matched
three-state confirmation is still evidence collection, so no learned-success claim is
made before its baseline/candidate outcomes and provenance checks complete.
